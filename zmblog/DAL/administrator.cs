...
namespace zmblog.DAL
{
	...
	public partial class administrator
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("adminID", "administrator"); 
		}
		...
		public int Add(zmblog.Model.administrator model)
		{
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@adminEmail", SqlDbType.VarChar,50),
		...
		public zmblog.Model.administrator GetModel(int adminID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 adminID,adminName,adminPwd,adminEmail,adminLevel from administrator ");
			strSql.Append(" where adminID=@adminID");
			SqlParameter[] parameters = {
					new SqlParameter("@adminID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public zmblog.Model.administrator DataRowToModel(DataRow row)
		{
			zmblog.Model.administrator model=new zmblog.Model.administrator();
			if (row != null)
			{
				...
				if(row["adminLevel"]!=null && row["adminLevel"].ToString()!="")
				{
					model.adminLevel=int.Parse(row["adminLevel"].ToString());
				}
			}
			return model;
		}
